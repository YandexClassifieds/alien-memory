...
trait LowLevelImplicits {
  ...
  ): Unwrap[Aligned[L, B], N := L1] = l => unwrap0(l.inner)

  implicit def alignedUnwrapUp[L <: Layout, L1, B <: Pow2](
    implicit
    unwrap0: Unwrap[L, L1],
  ): Unwrap[Aligned[L, B], L1] = l => unwrap0(l.inner)

  implicit def alignedUnwrapDown[L <: L1, L1 <: Layout, B <: Pow2]
    : Unwrap[Aligned[L, B], L1] = _.inner
  ...
  ): Unwrap[>>:[Head, L], N := L1] = s => unwrap(s.tail)

  implicit def recurseSUnwrap[L <: DynamicStruct, L1, Head <: :=[?, ?]](
    implicit
    unwrap: Unwrap[L, L1],
  ): Unwrap[>>:[Head, L], L1] = s => unwrap(s.tail)
